...
function fnStart() {
   ...
   document.querySelector(
      "#voting-double-container-card label span"
   ).innerHTML = TEXT_VOTING_DOUBLE;
   ...
   $("#about-link").html(TEXT_ABOUT_THIS_TOOL);
   $("#about-link").attr("href", aboutLink);

   const nodeImageInfoPopup = document.querySelector("#image-info-popup");
   if (imageInfoPopupPath) {
      nodeImageInfoPopup.setAttribute("href", imageInfoPopupLink);
      nodeImageInfoPopup
         .querySelector("img")
         .setAttribute("src", imageInfoPopupPath);
      nodeImageInfoPopup.querySelector("img").style.width = imageInfoPopupWidth;
   } else {
      nodeImageInfoPopup.remove();
   }
   ...
   if (!descriptionShowOnStart) {
     ...
     `;
      ...
      $("#descriptionExplanation")
         .empty()
         .append(descriptionExplanationContent);
...
function fnShowQuestionNumber(questionNumber) {
   ...
   if (questionNumber < arQuestionsLong.length) {
      ...
      if (animateQuestionsCard) {
         ...
         if (
            document.querySelector("#sectionDescription").style.display !==
            "none"
         ) {
            document
...
function fnJumpToQuestionNumber(questionNumber) {
   ...
   for (i = 1; i <= arQuestionsLong.length; i++) {
      ...
      let positionColor = fnTransformPositionToColor(
      ...
      if (arVotingDouble[i - 1]) {
         document.querySelector(`#jumpToQuestionNr${i}`).style.fontWeight =
            "bold";
      }
   }
}
         ...
         setTimeout(() => {
            document
               ...
               .forEach((row) => {
                  ...
                  row.querySelector(".logo-container").innerHTML =
                     `<img src="${arPartyLogosImg[partyNum]}" class='result-logo rounded img-fluid' alt="Logo ${arPartyNamesLong[partyNum]}" />`;
               });
         }, 0);
      });
...
function addContentToFinetuningTab() {
   document.querySelector("#finetuningHeading").innerHTML =
      `<h1>${TEXT_FINETUNING_HEADING}</h1><h2>${TEXT_FINETUNING_SUBHEADING}</h2>`;
   ...
   for (i = 0; i < intQuestions; i++) {
      ...
      tableContentResultsByThesis += `<div class='row border' id='resultsByThesisQuestion${i}Container' role='row'>
                <div class='col' id='resultsByThesisQuestion${i}' role='cell'>
                    ...
                    <div id="voting-double-container-question${i}">
                      ...
                      <button
                        class="bx bxs-help-circle icon-help icon-help-voting-double"
      ...
      <button class="bx bxs-info-circle icon-help" onclick="showModalResultDetails(${partyNum})"></button>
         ...
         .addEventListener("click", () => {
            function handleFullscreenPartyAnswerList() {
               ...
               if (btnExpand.classList.contains("expanded")) {
                  ...
                  containerQuestion.classList.add(
                  ...
                  setTimeout(() => {
                     ...
                     const heightOfQuestionTextContainer =
                        window.getComputedStyle(questionTextContainer).height;
                  ...
                  wrapperDiv.parentNode.insertBefore(
                     containerQuestion,
                     wrapperDiv
                  );
                  wrapperDiv.remove();
...
function generateSectionResults(arResults) {
   function addContentToInfoTab() {
      document.querySelector("#infoHeading").innerHTML =
         `<h1>${TEXT_INFO_HEADING}</h1><h2>${TEXT_INFO_SUBHEADING}</h2>`;
   ...
   function createNavigationBar() {
      function animateTabs(arTabs, oldActiveTab, newActiveTab) {
         ...
         setTimeout(() => {
            ...
            oldActiveTab.classList.remove(
               goRight ? "flyOutLeft" : "flyOutRight"
            );
